...
这是一个面向 AGI 的发育式学习架构。它起于一个判断：网络结构不该是人为设定的前提，而应当是学习的产物。种子神经元在环境信号驱动下展开维度、共发射成边、髓鞘化、分化分裂；所有学习信号来自环境即时反馈——没有反向传播，没有标注，也没有预训练嵌入。
...
真值探针方面：系统恢复了 9/16 条 FSM 转移，确定性转移全部命中；学完图片和电影沉积物之后再回访语法，棘轮保持率 1.02——技能没有打滑。
